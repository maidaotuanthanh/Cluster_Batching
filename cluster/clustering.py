...
def clustering_mapping(df, distance_threshold, dist_method, orders_number, wave_start, clust_start, df_type):
    ...
    # 1. Create Clusters
    list_coord, list_OrderNumber, clust_id, df = cluster_wave(df, distance_threshold, 'euclidian', clust_start, df_type)
...
def lines_mapping_clst(df, list_coord, list_OrderNumber, clust_id, orders_number, wave_start):
    ...
    # Dataframe mapping
    df.loc[:, 'ClusterID'] = df['OrderNumber'].map(dict_map)
...
def locations_listing(df_orderlines, wave_id):
    ...
    list_coord = list(set(tuple(coord) for coord in list_coord))
