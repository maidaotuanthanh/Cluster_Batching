...
def cluster_locations(list_coord, distance_threshold, dist_method, clust_start):
    """ Step 1: Create clusters of location"""
...
def cluster_wave(df, distance_threshold, dist_method, clust_start, df_type):
    ...
    # Mapping points
    df_map = pd.DataFrame(
        df.groupby(['OrderNumber', 'Coord_Cluster'])['SKU'].count()).reset_index()  # Here we use Coord Cluster
...
def lines_mapping(df, order_number, wave_start):
    ...
    df['WaveID'] = (df.OrderID % order_number == 0).shift(1).fillna(0).cumsum() + wave_start
    # Counting number of Waves
    waves_number = df.WaveID.max() + 1
    return df, waves_number


def lines_mapping_clst(df, list_coord, list_OrderNumber, clust_id, order_number, wave_start):
    ...
    # Dictionary for order mapping
    dict_omap = dict(zip(list_orders, [i for i in range(1, len(list_orders))]))
    # Order ID mapping
    df['OrderID'] = df['OrderNumber'].map(dict_omap)
    # Grouping Orders by Wave of orders_number
    df['WaveID'] = wave_start + ((df.OrderID % order_number == 0) | (df.ClusterID.diff() != 0)).shift(1).fillna(
        0).cumsum()
...
def locations_listing(df_orderlines, wave_id):
    ...
    list_coord = list(k for k in itertools.groupby(list_coord))
