...
def simulation_wave(y_low, y_high, origin_loc, orders_number, df_orderlines, list_wid, list_dst, list_route, list_ord):
    ...
    distance_route = 0
    # Create waves
    df_orderlines, waves_number = batch.mapping_batch.orderlines_mapping(df_orderlines, orders_number)
    for wave_id in range(waves_number):
        # Listing of all locations for this wave
        list_locs, n_locs, n_lines, n_pcs = batch.mapping_batch.locations_listing(df_orderlines, wave_id)
        # Results
        wave_distance, list_chemin = routing.routes.create_picking_route(origin_loc, list_locs, y_low, y_high)
...
def simulate_batch(n1, n2, y_low, y_high, origin_loc, orders_number, df_orderlines):
    ...
    for orders_number in range(n1, n2 + 1):
        list_wid, list_dst, list_route, list_ord, distance_route = simulation_wave(y_low, y_high, origin_loc,
                                                                                   orders_number,
